...
import 'package:flutter_cart/model/cart_model.dart' as fc;
import 'dart:convert';
...
class CartProvider extends ChangeNotifier {
  ...
  // Add a product to cart with minimal required fields
  void addProductToCart(Product product, {int quantity = 1}) {
    try {
      final price = (product.offerPrice ?? product.price ?? 0).toDouble();
      flutterCart.addToCart(
        cartModel: CartModel(
          ...
          // Provide at least one variant with price
          variants: [
            fc.ProductVariant(price: price),
          ],
          // Store product JSON as a string
          productDetails: jsonEncode(product.toJson()),
        ),
      );
      getCartItems();
      notifyListeners();
    } catch (e) {
      log('addProductToCart error: $e');
    }
  }
  ...
  Future<void> razorpayPayment({required void Function() operation}) async {
    try {
      if (razorpay == null) {
        SnackBarHelper.showErrorSnackBar(
            'Razorpay not available on this platform');
        return;
      }
      Response response =
