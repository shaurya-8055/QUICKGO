...
class DataProvider extends ChangeNotifier {
  ...
  List<ServiceRequest> get serviceRequests => _filteredServiceRequests;
  ...
  Future<List<ServiceRequest>> getAllServiceRequests({bool showSnack = false, String? status}) async {
    ...
    return _filteredServiceRequests;
  }

  void filterServiceRequests(String keyword) {
    if (keyword.isEmpty) {
      _filteredServiceRequests = List.from(_allServiceRequests);
    } else {
      final lower = keyword.toLowerCase();
      _filteredServiceRequests = _allServiceRequests.where((sr) {
        return (sr.customerName ?? '').toLowerCase().contains(lower) ||
            (sr.phone ?? '').toLowerCase().contains(lower) ||
            (sr.category ?? '').toLowerCase().contains(lower) ||
            (sr.status ?? '').toLowerCase().contains(lower);
      }).toList();
    }
    notifyListeners();
  }
