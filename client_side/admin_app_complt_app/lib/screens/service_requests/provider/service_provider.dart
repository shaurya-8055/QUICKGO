...
class ServiceProvider extends ChangeNotifier {
  ...
  if (assigneeName != null && assigneeName.isNotEmpty) 'assigneeName': assigneeName,
  if (assigneePhone != null && assigneePhone.isNotEmpty) 'assigneePhone': assigneePhone,
      ...
      if (res.isOk) {
        ...
        final msg = res.body?['message']?.toString() ?? res.statusText ?? 'Update failed';
        SnackBarHelper.showErrorSnackBar(msg);
        return (false, msg);
      }
    } catch (e) {
  ...
  Future<(bool, String)> deleteRequest(String id) async {
    try {
      _setSubmitting(true);
      final Response res = await _http.deleteItem(endpointUrl: 'service-requests', itemId: id);
      if (res.isOk) {
        final api = ApiResponse.fromJson(res.body, (obj) => obj);
        if (api.success) {
          SnackBarHelper.showSuccessSnackBar(api.message);
        ...
        final msg = res.body?['message']?.toString() ?? res.statusText ?? 'Delete failed';
        SnackBarHelper.showErrorSnackBar(msg);
        return (false, msg);
      }
    } catch (e) {
